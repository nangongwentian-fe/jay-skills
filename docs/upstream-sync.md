...
本仓库定期检查五个外部 skill 仓库。发现新提交后，同步对应插件并创建独立 PR；主分支不会被定时任务直接修改。
...
| `emilkowalski-skills` | `skills/`、`LICENSE` | 删除 Codex 不支持的 Claude 专属 frontmatter |
| `langchain-skills` | `config/skills/` | 将 `swarm` 的兼容性说明移入 Codex 支持的 metadata；上游无独立许可证文件 |
...
`emilkowalski-skills` 的 `review-animations` 使用 Claude 专属的 `disable-model-invocation` frontmatter。本仓使用 `patches/remove-unsupported-frontmatter.patch` 删除该字段，不修改 skill 正文。

`langchain-skills` 的 `swarm` 使用 Codex 不支持的顶层 `compatibility` frontmatter。本仓使用 `patches/move-swarm-compatibility-to-metadata.patch` 将原说明移入 `metadata.compatibility`，不修改 skill 正文。
